...
class Logger(logging.Logger):
    ...
    def log_criterion(self, update, cri, prefix = None):
        stat_str = []
        for key, value in cri.items():
            if key == "y_pre" or key == "y_true" or key.count("analysis") or key.count("dims"):
                continue
            else:
                if not type(value) is np.ndarray:
                    str_temp = " {}: {:.4f} ".format(key, value)
                else:
                    str_temp = " {}: {} ".format(key, ["{:.2f}".format(a) for a in value])
                stat_str.append(str_temp)
        stat_str = "\t".join(stat_str)
        if prefix != None:
            stat_str = "\t".join([prefix, stat_str])
        self.info("update: {}\t{}".format(update, stat_str))
...
def create_logger(cfg):
    return Logger(
        name=cfg.name, level=logging.INFO, format="%(asctime)-15s %(message)s"
    )
